...
void DoStateMachine(void) {
  ...
  case STATE_STARTUP:
    ...
    global_data_A36926.startup_counter = 0;
    while (global_data_A36926.control_state == STATE_STARTUP) {
      DoA36746();
      FlashLEDs();
  ...
  case STATE_NOT_READY:
    ...
    _CONTROL_NOT_READY = 1;
    while (global_data_A36926.control_state == STATE_NOT_READY) {
      DoA36746();
      if (ETMCanSlaveGetSyncMsgResetEnable()) {
	global_data_A36926.control_state = STATE_TESTING;
      }
    }
    break;
  ...
  case STATE_TESTING:
    _CONTROL_NOT_READY = 1;
    global_data_A36926.test_timer = 0;
    while (global_data_A36926.control_state == STATE_TESTING) {
      DoA36746();
...
void DoA36746(void) {
  ...
  UpdateFlowMeterInputCapture(&global_data_A36926.flow_meter_1_magnetron);
  UpdateFlowMeterInputCapture(&global_data_A36926.flow_meter_2_linac);
  UpdateFlowMeterInputCapture(&global_data_A36926.flow_meter_3_hv_tank);
  UpdateFlowMeterInputCapture(&global_data_A36926.flow_meter_4_hvps);
  UpdateFlowMeterInputCapture(&global_data_A36926.flow_meter_5_circulator);
  UpdateFlowMeterInputCapture(&global_data_A36926.flow_meter_6_alternate);
  ...
  if (_T1IF) {
    ...
    ETMAnalogScaleCalibrateADCReading(&global_data_A36926.analog_input_thermistor_1);
    ETMAnalogScaleCalibrateADCReading(&global_data_A36926.analog_input_thermistor_2);
    ETMAnalogScaleCalibrateADCReading(&global_data_A36926.analog_input_thermistor_3);
    ETMAnalogScaleCalibrateADCReading(&global_data_A36926.analog_input_SF6_pressure);

    // Convert the temperature reading voltage into temperature
    global_data_A36926.coolant_temperature_kelvin = ConvertThermistorVoltageToKelvin(global_data_A36926.analog_input_thermistor_1.reading_scaled_and_calibrated);
    ...
    //CalculateFlowMeterFlow(&global_data_A36926.flow_meter_3_hv_tank);
    //CalculateFlowMeterFlow(&global_data_A36926.flow_meter_4_hvps);
    //CalculateFlowMeterFlow(&global_data_A36926.flow_meter_5_circulator);
    //CalculateFlowMeterFlow(&global_data_A36926.flow_meter_6_alternate);


    // Update Temperature Switch Inputs
    ETMDigitalUpdateInput(&global_data_A36926.digital_input_7_cabinet_temp_switch, PIN_DIGITAL_INPUT_1_CABINET_TEMP_SWITCH);
    ETMDigitalUpdateInput(&global_data_A36926.digital_input_8_coolant_temp_switch, PIN_DIGITAL_INPUT_2_COOLANT_TEMP_SWITCH);
...
void InitializeA36746(void) {
  ...
  ETMAnalogInitializeInput(&global_data_A36926.analog_input_thermistor_1,
  ...
  ETMAnalogInitializeInput(&global_data_A36926.analog_input_thermistor_2,
			   MACRO_DEC_TO_SCALE_FACTOR_16(TEMPERATURE_SENSOR_FIXED_SCALE),
			   OFFSET_ZERO,
			   ANALOG_INPUT_2,
			   NO_OVER_TRIP,
  ...
  ETMAnalogInitializeInput(&global_data_A36926.analog_input_thermistor_3,
			   MACRO_DEC_TO_SCALE_FACTOR_16(TEMPERATURE_SENSOR_FIXED_SCALE),
			   OFFSET_ZERO,
			   ANALOG_INPUT_3,
			   NO_OVER_TRIP,
  ...
  ETMAnalogInitializeInput(&global_data_A36926.analog_input_SF6_pressure,
  ...
  ETMDigitalInitializeInput(&global_data_A36926.digital_input_7_cabinet_temp_switch, !ILL_TEMP_SWITCH_OVER_TEMP, TEMPERATURE_SWITCH_FILTER_TIME);
  ETMDigitalInitializeInput(&global_data_A36926.digital_input_8_coolant_temp_switch, !ILL_TEMP_SWITCH_OVER_TEMP, TEMPERATURE_SWITCH_FILTER_TIME);


  global_data_A36926.SF6_state = SF6_STATE_DELAY;
  global_data_A36926.SF6_fill_threshold = SF6_MINIMUM_TARGET_PRESSURE;
  global_data_A36926.SF6_low_pressure_override_counter = 0;
  global_data_A36926.SF6_state_delay_counter = 0;
  
  ETMEEPromReadPage(ETM_EEPROM_PAGE_COOLING_INTERFACE, 2, &global_data_A36926.SF6_pulses_available);
...
void FlashLEDs(void) {
  switch (((global_data_A36926.startup_counter >> 4) & 0b11)) {
...
void DoSF6Management(void) {
  switch (global_data_A36926.SF6_state) {
  ...
  case SF6_STATE_TEST:
    ...
    if (global_data_A36926.coolant_temperature_kelvin < MINIMUM_COOLANT_TEMP_FOR_SF6_MANAGEMENT) {
      _STATUS_SF6_COOLANT_TOO_LOW = 1;
      break;
    }
    _STATUS_SF6_COOLANT_TOO_LOW = 0;

    if ((global_data_A36926.analog_input_SF6_pressure.reading_scaled_and_calibrated < MINIMUM_PRESSURE_FOR_SF6_MANAGEMENT) && (global_data_A36926.SF6_low_pressure_override_counter == 0)) {
      _STATUS_SF6_PRESSURE_TO_LOW_TO_MANAGE = 1;
      break;
    }
    _STATUS_SF6_PRESSURE_TO_LOW_TO_MANAGE = 0;

    if (global_data_A36926.analog_input_SF6_pressure.reading_scaled_and_calibrated > global_data_A36926.SF6_fill_threshold) {
      _STATUS_SF6_FILL_REQUIRED = 0;
      break;
    }
    _STATUS_SF6_FILL_REQUIRED = 1;

    if (!global_data_A36926.SF6_pulses_available) {
    ...
    if (global_data_A36926.SF6_pulses_available) {
      global_data_A36926.SF6_pulses_available--;
    }

    if (global_data_A36926.SF6_low_pressure_override_counter) {
      global_data_A36926.SF6_low_pressure_override_counter--;
    }

    if (global_data_A36926.SF6_bottle_pulses_remaining) {
    ...
    global_data_A36926.SF6_state = SF6_STATE_CHARGING;

    break;


  case SF6_STATE_CHARGING:
    global_data_A36926.SF6_state_charging_counter++;
    PIN_RELAY_OUT_SF6_SOLENOID = OLL_CLOSE_SOLENOID;
    _STATUS_SF6_SOLENOID_RELAY_CLOSED = 1;
    _STATUS_SF6_FILLING = 1;
    if (global_data_A36926.SF6_state_charging_counter >= SF6_TIME_CHARGING) {
      global_data_A36926.SF6_state_delay_counter = 0;
      global_data_A36926.SF6_state = SF6_STATE_DELAY;
    }
    break;
  ...
  case SF6_STATE_DELAY:
    ...
    _STATUS_SF6_FILLING = 1;
    global_data_A36926.SF6_state_delay_counter++;
    if (global_data_A36926.SF6_state_delay_counter >= SF6_TIME_DELAY) {
      _STATUS_SF6_FILLING = 0;
      global_data_A36926.SF6_state = SF6_STATE_TEST;
...
void __attribute__((interrupt, no_auto_psv)) _ADCInterrupt(void) {
  ...
  if (_BUFS) {
    // read ADCBUF 0-7
    global_data_A36926.analog_input_thermistor_1.adc_accumulator        += ADCBUF0;
    global_data_A36926.analog_input_thermistor_2.adc_accumulator        += ADCBUF1;
    global_data_A36926.analog_input_thermistor_3.adc_accumulator        += ADCBUF2;
    global_data_A36926.analog_input_SF6_pressure.adc_accumulator        += ADCBUF3;
    ...
    global_data_A36926.adc_test                                          = (ADCBUF7 << 4);
  } else {
    // read ADCBUF 8-15
    global_data_A36926.analog_input_thermistor_1.adc_accumulator        += ADCBUF8;
    global_data_A36926.analog_input_thermistor_2.adc_accumulator        += ADCBUF9;
    global_data_A36926.analog_input_thermistor_3.adc_accumulator        += ADCBUFA;
    global_data_A36926.analog_input_SF6_pressure.adc_accumulator        += ADCBUFB;
    ...
    global_data_A36926.adc_test                                          = (ADCBUFF << 4);
  }
  ...
  if (global_data_A36926.accumulator_counter >= 128) {
    ...
    global_data_A36926.analog_input_thermistor_1.adc_accumulator >>= 3;  // This is now a 16 bit number average of previous 128 samples 
    global_data_A36926.analog_input_thermistor_1.filtered_adc_reading = global_data_A36926.analog_input_thermistor_1.adc_accumulator;
    global_data_A36926.analog_input_thermistor_1.adc_accumulator = 0;

    global_data_A36926.analog_input_thermistor_2.adc_accumulator >>= 3;  // This is now a 16 bit number average of previous 128 samples 
    global_data_A36926.analog_input_thermistor_2.filtered_adc_reading = global_data_A36926.analog_input_thermistor_2.adc_accumulator;
    global_data_A36926.analog_input_thermistor_2.adc_accumulator = 0;

    global_data_A36926.analog_input_thermistor_3.adc_accumulator >>= 3;  // This is now a 16 bit number average of previous 128 samples 
    global_data_A36926.analog_input_thermistor_3.filtered_adc_reading = global_data_A36926.analog_input_thermistor_3.adc_accumulator;
    global_data_A36926.analog_input_thermistor_3.adc_accumulator = 0;

    global_data_A36926.analog_input_SF6_pressure.adc_accumulator >>= 3;  // This is now a 16 bit number average of previous 128 samples 
    global_data_A36926.analog_input_SF6_pressure.filtered_adc_reading = global_data_A36926.analog_input_SF6_pressure.adc_accumulator;
    global_data_A36926.analog_input_SF6_pressure.adc_accumulator = 0;
